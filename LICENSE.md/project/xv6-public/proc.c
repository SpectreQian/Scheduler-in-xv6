      ...
      if(totalclick>=250)
				...
				if(c3!=-1)
        {
            ...
            c0 = c0 + c3 + 1;
				...
				totalclick=0;
				cprintf("PID\tP_Name   \tP_State\tP_Priority\tP_Clicks\tReset\tP_tickets\n");
        		for(pr = ptable.proc; pr < &ptable.proc[NPROC]; pr++)
    			{
    				if(pr->pid != 0) 
    					cprintf("%d\t%s      \t%d\t%d\t\t%d\t\t%d\t%d\n", pr->pid, pr->name,pr->state,pr->priority,pr->clicks,reset,pr->tickets);
      			}
				break;

			}

			if(p->clicks ==Timeslice[0])
			{
				//cprintf("1->2\n");
      ...
      if(totalclick>=250)
		...
		while(c3!=-1)
		{
          ...
          q3[i3]=0;
			...
			switchuvm(p);
			p->state = RUNNING;
			//("4-%d:pid=%d; p_name=%s; p_click=%d,c3=%d\n",i3,p->pid,p->name, p->clicks,c3);
			//cprintf("%d,%d,%d,%d\n", c0, c1, c2, c3);
			swtch(&cpu->scheduler, proc->context);
			switchkvm();
	...
	yield(void)
{
	...
	proc->state = RUNNABLE;
	proc->tickets = 1000/proc->pid+1000;
	sched();
	release(&ptable.lock);
}
...
pstats(void)
{
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
      ...
      if(p->pid != 0) 
        cprintf("%d\t%s      \t%d\t%d\t\t%d\t\t%d\t%d\n", p->pid, p->name,p->state,p->priority,p->clicks,reset,p->tickets);
